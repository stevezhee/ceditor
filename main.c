...
bool isLidentChar(char c)
{
  return (c >= 'a' && c <= 'z') || c == '_';
}
bool isUidentChar(char c)
{
  return c >= 'A' && c <= 'Z';
}
...
color_t getCharColor(char c, tokSt_t *accp, char *p, char *q)
{
  ...
  case IN_SLCOMMENT:
    ...
    return commentColor;
  case IN_MLCOMMENT:
      if (c == '*') *accp = IN_MLCOMMENT_END;
      return commentColor;
  case IN_MLCOMMENT_END:
    if (c == '/') *accp = TOKBEGIN;
    else *accp = IN_MLCOMMENT;
    return commentColor;
  case IN_LIDENT:
    if (isIdentChar(c)) return lidentColor;
    goto tok_begin;
  case IN_UIDENT:
    if (isIdentChar(c)) return uidentColor;
    goto tok_begin;
  case IN_PREPROC:
    if (isIdentChar(c)) return preprocColor;
    goto tok_begin;
  case IN_NUMBER:
    if (isIdentChar(c)) return literalColor;
    goto tok_begin;
  default:
    goto tok_begin;
  }
